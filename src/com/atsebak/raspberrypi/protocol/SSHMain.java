...
//  }
     ...
     * @throws java.io.IOException if there is a problem with reading key
     */
     ...
     * @throws java.io.IOException if loading configuration file failed
     */
    private static SSHMain parseArguments(String[] args) throws IOException {
        if (args.length != 2 && args.length != 4) {
            System.err.println(PIBundle.message("sshmain.invalid.amount.of.arguments", Arrays.asList(args)));
            System.exit(1);
        }
        int i = 0;
        Integer port = null;
     ...
     * the method throws {@link java.io.IOException}.
     *
     * @param c the connection to use for authentication
     * @throws java.io.IOException in case of IO error or authentication failure
     */
            ...
            if (PUBLIC_KEY_METHOD.equals(method)) {
                ...
                for (int i = myHost.getNumberOfPasswordPrompts(); i > 0; i--) {
                    ...
                    if (c.authenticateWithKeyboardInteractive(myHost.getUser(), interactiveSupport)) {
                        ...
                        myLastError = PIBundle.getString("sshmain.keyboard.interactive.failed");
                    }
                ...
                for (int i = 0; i < myHost.getNumberOfPasswordPrompts(); i++) {
                    ...
                    if (password == null) {
                        ...
                        if (c.authenticateWithPassword(myHost.getUser(), password)) {
                            ...
                            myLastError = PIBundle.getString("sshmain.password.failed");
                        }
                    }
                }
            }
            ...
            if (file.exists()) {
                ...
                if (isEncryptedKey(text)) {
                    ...
                    for (i = 0; i < myHost.getNumberOfPasswordPrompts(); i++) {
                        ...
                        if (passphrase == null) {
                            ...
                            try {
                                ...
                                // decoding failed
                                myLastError = PIBundle.message("sshmain.invalidpassphrase", keyPath);
                                myErrorCause = e;
                                continue;
                            }
                            break;
                    ...
                    if (i == myHost.getNumberOfPasswordPrompts()) {
                        myLastError = PIBundle.message("sshmain.too.mush.passphrase.guesses", keyPath, myHost.getNumberOfPasswordPrompts());
                        return false;
                    }
                }
                // try authentication
                if (c.authenticateWithPublicKey(myHost.getUser(), text, passphrase)) {
                    ...
                    if (passphrase != null) {
                        ...
                        myLastError = PIBundle.message("sshmain.pk.authenitication.failed", keyPath);
                    } else {
                        myLastError = "";
                    }
                }
        ...
        final Runnable action = new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    System.err.println(PIBundle.message("sshmain.forwarding.failed", name, e.getMessage()));
                    e.printStackTrace();
                    myExitCode = 1;
                    if (releaseSemaphore) {
     ...
     * @throws java.io.IOException if there is a IO problem
     */
        ...
        public boolean verifyServerHostKey(String hostname, int port, String serverHostKeyAlgorithm, byte[] serverHostKey) throws Exception {
            ...
            try {
                ...
                if (keyCheck) {
                    ...
                    System.err.println(PIBundle.message("sshmain.invald.host.key", serverHostKeyAlgorithm, fingerprint));
                    return false;
                }
            } catch (Throwable t) {
                System.err.println(PIBundle.message("sshmain.failed.to.verify.key", t.getMessage()));
                t.printStackTrace();
                return false;
            }
        }
