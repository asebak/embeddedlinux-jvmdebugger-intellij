...
import com.atsebak.raspberrypi.config.SSHConnectionSettings;
...
public class GitSSHGUIHandler {
    ...
    public boolean verifyServerHostKey(final String hostname,
        ...
        if (isNew) {
            message = PIBundle.message("ssh.new.host.key", hostname, port, fingerprint, serverHostKeyAlgorithm);
        } else {
            message = PIBundle.message("ssh.changed.host.key", hostname, port, fingerprint, serverHostKeyAlgorithm);
        }
        final AtomicBoolean rc = new AtomicBoolean();
        ApplicationManager.getApplication().invokeAndWait(new Runnable() {
            public void run() {
                rc.set(Messages.YES == Messages.showYesNoDialog(myProject, message, PIBundle.getString("ssh.confirm.key.titile"), null));
            }
        }, ModalityState.any());
        return rc.get();
    }
    ...
    public String askPassphrase(final String username, final String keyPath, boolean resetPassword, final String lastError) {
        ...
        return PasswordSafePromptDialog.askPassphrase(myProject, PIBundle.getString("ssh.ask.passphrase.title"),
                PIBundle.message("ssh.askPassphrase.message", keyPath, username),
    ...
    private void showError(final String lastError) {
        if (lastError.length() != 0) {
            Messages.showErrorDialog(myProject, lastError, PIBundle.getString("ssh.error.title"));
        }
    }
    ...
    public String askPassword(final String username, boolean resetPassword, final String lastError) {
        String error = processLastError(resetPassword, lastError);
        return PasswordSafePromptDialog.askPassword(myProject, PIBundle.getString("ssh.password.title"),
                PIBundle.message("ssh.password.message", username),
    ...
    private class GitSSHKeyboardInteractiveDialog extends DialogWrapper {
        ...
        public GitSSHKeyboardInteractiveDialog(String name,
            ...
            myEcho = echo;
            myUserName = userName;
            setTitle(PIBundle.message("ssh.keyboard.interactive.title", name));
            init();
            setResizable(true);
            setModal(true);
        }
        ...
        protected JComponent createCenterPanel() {
            if (contents == null) {
                ...
                c.gridx = 0;
                c.gridy = line;
                contents.add(new JLabel(PIBundle.getString("ssh.keyboard.interactive.username")), c);
                c = new GridBagConstraints();
                c.insets = insets;
                c.gridx = 1;
                c.gridy = line;
